...
class bagWalletLayOut extends ListLayout{
    public $dataNum ;
    ...
    public function accessClothesTable()
    {
        global $clothesDbConnection;
        $conn= $clothesDbConnection;
        $query = "SELECT * FROM Clothes WHERE (category1 = '가방/지갑') ORDER BY id DESC LIMIT $this->startPoint,$this->dataNumForPage  ";
        $result = mysqli_query($conn, $query);
        return $result;
    }
}
...
?>
<script>
    document.getElementById("navTitle").textContent = "가방/지갑";
</script>
</body>
</html>
