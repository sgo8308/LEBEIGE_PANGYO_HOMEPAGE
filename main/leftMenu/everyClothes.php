...
<head>
    ...
    <link rel="stylesheet" href="..\css\everyClothes.css?10">
...
class everyClothesLayOut extends ListLayout{
    ...
    public function accessClothesTable()
    {
        global $clothesDbConnection;
        $conn= $clothesDbConnection;
        $query = "SELECT * FROM Clothes ORDER BY id DESC LIMIT $this->startPoint,$this->dataNumForPage ";
//        $query = "SELECT * FROM Clothes ORDER BY id DESC";
        $result = mysqli_query($conn, $query);
        return $result;
    }
...
?>
<script>
    document.getElementById("navTitle").textContent = "전체 상품";
</script>
