...
class popularClothesLayOut extends ListLayout{
    public $dataNum ;
    ...
    public function accessClothesTable()
    {
        global $clothesDbConnection;
        $conn= $clothesDbConnection;
        $query = "SELECT * FROM Clothes WHERE (category3 = '인기상품') ORDER BY id DESC LIMIT $this->startPoint,$this->dataNumForPage";
        $result = mysqli_query($conn, $query);
        return $result;
    }
...
?>
<script>
    document.getElementById("navTitle").textContent = "인기상품";
</script>
</body>
</html>
