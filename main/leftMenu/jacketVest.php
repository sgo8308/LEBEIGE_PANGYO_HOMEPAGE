...
class jacketVestLayOut extends ListLayout{
    public $dataNum ;
    ...
    public function accessClothesTable()
    {
        global $clothesDbConnection;
        $conn= $clothesDbConnection;
        $query = "SELECT * FROM Clothes WHERE (category1 = '재킷/베스트') ORDER BY id DESC LIMIT $this->startPoint,$this->dataNumForPage ";
        $result = mysqli_query($conn, $query);
        return $result;
    }
...
?>
<script>
    document.getElementById("navTitle").textContent = "재킷/베스트";
</script>
</body>
</html>
